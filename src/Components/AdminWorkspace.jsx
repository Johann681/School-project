...
import { CalendarDays, Copy, LayoutDashboard, Plus, RefreshCcw, School, Trash2, UserPlus, Users, X } from "lucide-react";
...
const AdminWorkspace = () => {
  ...
  const regenerateStudentCode = async (student) => {
    if (!window.confirm(`Generate a new parent-link code for ${student.fullName}? The old code will stop working.`)) return;
    setWorking(true);
    try {
    ...
    } catch (error) { notify(error.response?.data?.message || "Unable to regenerate student code.", "error"); }
    finally { setWorking(false); }
  };
  ...
  const pendingStudents = data.students.filter((student) => !student.isActivated);
  const studentsWithoutClass = data.students.filter((student) => !student.studentClass);
  ...
  return <DashboardLayout role="Administration" title="School control center" subtitle="Manage people, academic structure, and the school timetable from one focused workspace." userName={session?.name || session?.email} notifications={[pendingStudents.length && { title: "Pending activations", message: `${pendingStudents.length} student account${pendingStudents.length === 1 ? "" : "s"} need review.` }, studentsWithoutClass.length && { title: "Class assignments", message: `${studentsWithoutClass.length} student${studentsWithoutClass.length === 1 ? "" : "s"} still need a class.` }, !timetableReady && { title: "Timetable setup", message: "Set up classes and assignments before publishing the timetable." }].filter(Boolean)} onLogout={() => { localStorage.removeItem("lmsAuth"); window.location.href = "/login"; }} actions={<button type="button" onClick={loadData} className="dash-btn-secondary"><RefreshCcw className="h-4 w-4" /> Refresh</button>}>
    {section === "overview" && <Panel title="Needs attention" description="Review these items before the school is ready to run."><div className="grid gap-3 sm:grid-cols-3"><button type="button" onClick={() => { setSection("people"); setSearch(""); }} className="rounded-xl border border-slate-200 p-4 text-left transition hover:border-amber-300 hover:bg-amber-50"><p className="text-2xl font-bold text-slate-900">{pendingStudents.length}</p><p className="mt-1 text-sm font-semibold text-slate-700">Pending activations</p><p className="mt-1 text-xs text-slate-500">Review student access</p></button><button type="button" onClick={() => { setSection("people"); setSearch(""); }} className="rounded-xl border border-slate-200 p-4 text-left transition hover:border-indigo-300 hover:bg-indigo-50"><p className="text-2xl font-bold text-slate-900">{studentsWithoutClass.length}</p><p className="mt-1 text-sm font-semibold text-slate-700">Students without class</p><p className="mt-1 text-xs text-slate-500">Assign before registration</p></button><button type="button" onClick={() => setSection("timetable")} className="rounded-xl border border-slate-200 p-4 text-left transition hover:border-emerald-300 hover:bg-emerald-50"><p className="text-2xl font-bold text-slate-900">{timetableReady ? "Ready" : "Not ready"}</p><p className="mt-1 text-sm font-semibold text-slate-700">Timetable status</p><p className="mt-1 text-xs text-slate-500">{timetableReady ? "Schedule is published" : "Generate a schedule"}</p></button></div></Panel>}
    ...
    {section === "people" && !loading && <Panel title="Student parent codes" description="Copy a code for a parent or generate a replacement when one has been forgotten."><div className="grid gap-3 sm:grid-cols-2">{filteredStudents.map((student) => <div key={student._id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800">{student.fullName}</p><p className="font-mono text-xs text-slate-500">{student.studentCode || "No code generated"}</p></div><div className="flex shrink-0 gap-1"><button type="button" disabled={!student.studentCode} onClick={() => navigator.clipboard?.writeText(student.studentCode).then(() => notify("Student code copied."))} className="rounded-lg p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-800 disabled:opacity-40" aria-label={`Copy code for ${student.fullName}`}><Copy className="h-4 w-4" /></button><button type="button" disabled={working} onClick={() => regenerateStudentCode(student)} className="rounded-lg px-2 py-1.5 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-50 disabled:opacity-40">Regenerate</button></div></div>)}</div></Panel>}
